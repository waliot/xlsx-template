...
module.exports = (function() {
    ...
    /**
     * Attributes:
     * - r = Reference (cell reference: A set of coordinates that a cell occupies on a worksheet. For example, "B3" is the reference of a cell that appears at the intersection of column "B" and row "3".)
     * - s = Style Index (style: A set of formatting options that is applied to text, tables, charts, and other objects in a document)
     * - t = Cell Data Type (data type: A property of a field that defines the kind of data that is stored in the field, or defines the kind of data returned by an expression when the expression is evaluated.)
     *
     * https://docs.microsoft.com/ru-ru/dotnet/api/documentformat.openxml.spreadsheet.cellvalues?view=openxml-2.8.1
     * https://stackoverflow.com/a/18346273
     */
    ...
    */
    Workbook.prototype.copySheet = function(sheetName, copyName, binary = true){
      var self = this;
    ...
    */
    ...
    Workbook.prototype.substitute = function(sheetName, substitutions) {
        ...
        const columnStyles = [];
        const columns = self.sheet.root._children.find(element => element.tag === 'cols');
        if (columns) {
            columns._children.forEach(element => {
                const style = element.attrib.style;

                const minIndex = element.attrib.min;
                const maxIndex = element.attrib.max;
                const count = parseInt(maxIndex, 10) - parseInt(minIndex, 10) + 1;
                if (count > columns._children.length) {
                    return;
                }
                ...
                if(cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(function(placeholder) {
                        ...
                        if(placeholder.full && placeholder.type === "table" && substitution instanceof Array) {
                            ...
                            newCellsInserted = self.substituteArray(cells, cell, substitution, placeholder);
    ...
    Workbook.prototype.insertCellValue = function(cell, substitution) {
        ...
        } else if (typeof(substitution) === "string" && substitution.match(/^[+-]?\d+([.,]\d+)?$/)) {
            delete cell.attrib.t; // type n (number) by default
            cellValue.text = substitution.replace(/,/g, '.');
    ...
    Workbook.prototype.substituteArray = function(cells, cell, substitution, placeholder) {
        var self = this;
        ...
        substitution.forEach(function(substitutionValue) {
            ++newCellsInserted;
            ...
            self.insertCellValue(newCell, substitutionValue);

            newCell.attrib.r = currentCell;
            if (placeholder.placeholder === "${table:rows.values}") {
                newCell.attrib.s = self.takeCellStyle(currentCell, cell.attrib.s);
            }
            cells.push(newCell);
        });
    ...
    Workbook.prototype.takeCellStyle = function (cellRef, defaultStyle) {
        let self = this;

        const columnName = cellRef.replace(/[^A-Z\s!?]/g, '');
        const columnIndex = self.charToNum(columnName) - 1;
        const columnStyle = self.columnStyles[columnIndex];
        if (columnStyle) {
            return columnStyle;
        } else {
            return defaultStyle;
            ...
            substitution.forEach(function(element, idx) {
                ...
                if(idx === 0) { // insert in the row where the placeholders are
                    ...
                    if(value instanceof Array) {
                        newCellsInserted = self.substituteArray(cells, cell, value, placeholder);
                    ...
                    if(value instanceof Array) {
                        newCellsInsertedOnNewRow = self.substituteArray(newCells, newCell, value, placeholder);
